...
import { useEmailVerifyViewModel } from '../../../viewmodels/EmailVerifyViewModel';
...
export const EmailVerifyScreen = () => {
  ...
  const handleButtonConfirm = async () => {
    ...
    const result = await handleConfirm(user?.email)
    if (result) {
    ...
    <>
      ...
      <View style={styles.container}>
        ...
        <View style={{flexDirection: 'row', gap: 8}}>
          <TouchableOpacity
            disabled={timecode > 0}
            onPress={() => user && resendCode(user.email, user.name, user.lastname)}
