...
import { AuthTokensInterface, UserLoginResponseInterface } from "../../../domain";


export function useAuth() {
  const { setStatus, setTokens, setUser } = useContext(AuthContext)

  const loginUser = useCallback(async (user: UserLoginResponseInterface, tokens: AuthTokensInterface) => {
