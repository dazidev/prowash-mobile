...
import { regularExp } from "../../interfaces/config/regular-exp"
...
const ChangePasswordViewModel = () => {
  ...
  useEffect(() => {
    ...
    if (!regularExp.password.test(fieldValue.newPassword)) return setChanges(false)
    if (fieldValue.newPassword === fieldValue.password) return setChanges(false)
    if (fieldValue.newPassword !== fieldValue.repeatNewPassword) return setChanges(false)
    return setChanges(true)
  }, [fieldValue.password, fieldValue.newPassword, fieldValue.repeatNewPassword])
  ...
  const verifiedFormatPassword = (value: string, field: FieldKey) => {
    if (field === 'password' || field === 'newPassword'){
      if (!regularExp.password.test(value)){
        setFieldError((prev) => ({...prev, [field]: '*The password must be at least 8 characters long and include an uppercase letter, a lowercase letter, a number, and a symbol.'}))
      }
    } else if (field === 'repeatNewPassword') {
      if (fieldValue.newPassword !== value) {
        setFieldError((prev) => ({...prev, [field]: "*The new passwords don't match. Make sure both fields are identical."}))
      }
    }
  }
  ...
  const saveChanges = async (): Promise<{success: boolean, message?: string}> => {
    const response = await UserService.changeUserPassword(user?.id as string, fieldValue.password, fieldValue.newPassword, fieldValue.repeatNewPassword)
